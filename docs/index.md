...
- **🪐 Multi-Domain & Mirrors Support**: Automatically checks and resolves active official domains via `https://www.animesaturn.me/` (`animesaturn.net`, `animesaturn.tv`, `animesaturn.in`, `animemars.org`, etc.).
...
- **📥 Robust Downloader**: High-speed chunked streaming downloads with resume support, range requests, customizable progress bars (`tqdm`), and interruptible callbacks.
- **💻 Full CLI Tool**: Built-in `animesaturn` command-line utility for search, inspecting anime, and direct downloads.
